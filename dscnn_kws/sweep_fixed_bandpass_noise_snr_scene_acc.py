...
OUT_TRAIN_CSV = Path("bandpass_pwl_snr_scene_arch_sweep_train_results.csv")
OUT_GRID_CSV = Path("bandpass_pwl_snr_scene_arch_sweep_grid_results.csv")
OUT_SCENE_SUMMARY_CSV = Path("bandpass_pwl_snr_scene_arch_sweep_scene_summary.csv")
OUT_ARCH_SUMMARY_CSV = Path("bandpass_pwl_snr_scene_arch_sweep_arch_summary.csv")
BEST_MODEL_DIR = Path("dscnn_kws") / "runs" / "bandpass_pwl_snr_scene_arch_sweep_best_models"
WRITE_OUTPUTS = True
...
def copy_best_model(args, output: str, dataset: str, arch_name: str, num_layers: int, channels: int):
    ...
    out_path = BEST_MODEL_DIR / (
        ...
        f"params{params}_bandpass{args.bandpass_n_bands}_{args.log_approx_mode}_noise_best.pt"
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Sweep DSCNN sizes with Conv1D bandpass frontend, then test ACC across fixed SNRs and TAU scenes."
    )
    ...
    parser.add_argument(
        "--out_prefix",
        default="bandpass_pwl_snr_scene_arch_sweep",
...
def main():
    args = parse_args()
    if args.dct_coeff != args.bandpass_n_bands:
        raise ValueError(
            "Bandpass frontend requires --dct_coeff to equal --bandpass_n_bands. "
