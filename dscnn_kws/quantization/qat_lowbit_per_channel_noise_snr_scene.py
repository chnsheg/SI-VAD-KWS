...
# Quantization. Set BIT_WIDTH to 4 or 3.
BIT_WIDTH = 4
...
PATTERN = "*.pt"
CHECKPOINTS = None
OUTPUT_DIR = f"./dscnn_kws/quantization/qat_int{BIT_WIDTH}_per_channel_noise_snr_scene_models"
TRAIN_RESULTS_CSV = f"./dscnn_kws/quantization/qat_int{BIT_WIDTH}_per_channel_noise_snr_scene_train_results.csv"
GRID_RESULTS_CSV = f"./dscnn_kws/quantization/qat_int{BIT_WIDTH}_per_channel_noise_snr_scene_grid_results.csv"
LIMIT = 0
...
# QAT. PyTorch has no true INT4/INT3 convert backend; this script uses
# low-bit fake-quant QAT and exports qint tensors for reference/hardware work.
# Compared with qat_lowbit_noise_snr_scene.py, this variant trains longer and
# uses per-channel symmetric weight quantization for Conv/Linear weights.
QAT_EPOCHS = 30
LR = 5e-5
WEIGHT_DECAY = 1e-6
BACKEND = "fbgemm"
...
def signed_qrange(bit_width: int) -> tuple[int, int]:
    if bit_width not in SUPPORTED_LOW_BITS:
        raise ValueError(f"Only int3/int4 are supported here, got bit_width={bit_width}")
    return -(1 << (bit_width - 1)), (1 << (bit_width - 1)) - 1


def format_name(bit_width: int) -> str:
    return f"int{bit_width}"


def default_output_dir(bit_width: int) -> str:
    return f"./dscnn_kws/quantization/qat_int{bit_width}_per_channel_noise_snr_scene_models"


def default_train_csv(bit_width: int) -> str:
    return f"./dscnn_kws/quantization/qat_int{bit_width}_per_channel_noise_snr_scene_train_results.csv"


def default_grid_csv(bit_width: int) -> str:
    return f"./dscnn_kws/quantization/qat_int{bit_width}_per_channel_noise_snr_scene_grid_results.csv"


def make_symmetric_lowbit_fake_quant(bit_width: int) -> FakeQuantize:
...
def make_per_channel_lowbit_weight_fake_quant(bit_width: int) -> FakeQuantize:
    qmin, qmax = signed_qrange(bit_width)
    return FakeQuantize.with_args(
...
def make_lowbit_qconfig(bit_width: int) -> QConfig:
    ...
    weight_fake_quant = make_per_channel_lowbit_weight_fake_quant(bit_width)
    return QConfig(activation=activation_fake_quant, weight=weight_fake_quant)


class LowBitBackboneWrapper(nn.Module):
    """Keep fake-quant active around the backbone for low-bit QAT evaluation."""

    def __init__(self, backbone: nn.Module, bit_width: int):
        super().__init__()
        self.input_fake_quant = make_symmetric_lowbit_fake_quant(bit_width)
        self.backbone = backbone
        self.output_fake_quant = make_symmetric_lowbit_fake_quant(bit_width)
...
class FrontendLowBitQATWrapper(nn.Module):
    """Add signed symmetric int3/int4 fake-quant points around the frontend path."""
    ...
    def __init__(self, model: nn.Module, bit_width: int):
        ...
        self.waveform_fake_quant = make_symmetric_lowbit_fake_quant(bit_width)
        self.pre_emphasis_fake_quant = make_symmetric_lowbit_fake_quant(bit_width)
        self.frontend_output_fake_quant = make_symmetric_lowbit_fake_quant(bit_width)
        self.backbone_input_fake_quant = make_symmetric_lowbit_fake_quant(bit_width)
...
def prepare_model_for_lowbit_qat(model: nn.Module, args: argparse.Namespace) -> nn.Module:
    if args.quantize_frontend:
        model = FrontendLowBitQATWrapper(model, args.bit_width)
    if not hasattr(model, "backbone"):
        raise AttributeError("Low-bit QAT script expects model.backbone")
    fuse_backbone_for_qat(model.backbone)
    model.backbone = LowBitBackboneWrapper(model.backbone, args.bit_width)
    model.qconfig = None
    model.backbone.qconfig = make_lowbit_qconfig(args.bit_width)
    model.train()
...
def export_lowbit_state_dict(model: nn.Module, bit_width: int) -> tuple[dict[str, torch.Tensor], dict[str, dict[str, Any]]]:
    q_state: dict[str, torch.Tensor] = {}
    q_specs: dict[str, dict[str, Any]] = {}
    qmin, qmax = signed_qrange(bit_width)
    for key, value in model.state_dict().items():
        if torch.is_tensor(value) and torch.is_floating_point(value):
            if is_per_channel_weight_tensor(key, value):
...
def save_lowbit_artifacts(
    *,
    ...
    tag = format_name(args.bit_width)
    qat_path = output_dir / f"{stem}_qat_{tag}_per_channel_prepared_best.pt"
    fake_quant_path = output_dir / f"{stem}_qat_{tag}_per_channel_fake_quant.pt"
    qint_state, q_specs = export_lowbit_state_dict(qat_model, args.bit_width)
    qmin, qmax = signed_qrange(args.bit_width)
    common = {
        ...
        "qat": {
            ...
            "scope": (
                f"frontend {tag} activation fake-quant QAT + DSCNN backbone {tag} "
                "activation fake-quant with per-channel low-bit weights"
                if args.quantize_frontend
                else f"DSCNN backbone {tag} activation fake-quant with per-channel low-bit weights; frontend remains float"
            ),
        },
        "note": (
            "PyTorch eager-mode quantized kernels are INT8-only. This checkpoint "
            "keeps fake-quantized float modules for evaluation and stores qint tensors "
            "for low-bit export/reference."
        ),
    }
    torch.save({**common, "state_dict": qat_model.state_dict(), "format": f"prepared_qat_{tag}_per_channel_weight_float"}, qat_path)
    torch.save(
        {
            **common,
            ...
            "quant_specs": q_specs,
            "format": f"fake_quant_{tag}_per_channel_weight_with_qint_export",
        },
        fake_quant_path,
    )
    ...
    for scene_idx, scene in enumerate(args.scene_names):
        ...
        for snr_idx, snr_db in enumerate(args.test_snrs):
            ...
            row = {
                ...
                "activation_qscheme": "per_tensor_symmetric",
                "weight_qscheme": "per_channel_symmetric",
                "quantized_backend": "per_channel_weight_fake_quant_float_ops",
            }
            rows.append(row)
            print(
                f"[GRID-{format_name(args.bit_width).upper()}] {dataset} | {arch} | "
...
def run_one_checkpoint(path: Path, args: argparse.Namespace, device: torch.device) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    model = prepare_model_for_lowbit_qat(model, args).to(device)
    ...
    lowbit_model = copy.deepcopy(model).cpu().eval()
    lowbit_test_m = evaluate(lowbit_model, test_list_loader, torch.device("cpu"))
    ...
    artifact_row = {
        ...
        "quantized_test_tau_list_acc": lowbit_test_m.acc,
        "quantized_test_tau_list_f1": lowbit_test_m.f1,
        "bit_width": args.bit_width,
        "quant_format": tag,
        "activation_qscheme": "per_tensor_symmetric",
        "weight_qscheme": "per_channel_symmetric",
    ...
    qat_path, fake_quant_path = save_lowbit_artifacts(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            f"INT{BIT_WIDTH} extended QAT fake-quant fine-tuning for noise-scene DSCNN checkpoints "
            "with per-channel symmetric Conv/Linear weights."
        )
    )
    parser.add_argument("--bit_width", type=int, choices=sorted(SUPPORTED_LOW_BITS), default=BIT_WIDTH)
    ...
    # Keep default output names aligned with an explicitly changed bit width.
    if args.bit_width != BIT_WIDTH:
        if args.output_dir == OUTPUT_DIR:
            args.output_dir = default_output_dir(args.bit_width)
        if args.train_results_csv == TRAIN_RESULTS_CSV:
            args.train_results_csv = default_train_csv(args.bit_width)
        if args.grid_results_csv == GRID_RESULTS_CSV:
            args.grid_results_csv = default_grid_csv(args.bit_width)
    return args


def validate_args(args: argparse.Namespace) -> None:
    signed_qrange(args.bit_width)
...
def main() -> None:
    ...
    print(
        ...
        f"quant={format_name(args.bit_width)}, qrange={signed_qrange(args.bit_width)}"
    )
    ...
    save_csv(train_rows, Path(args.train_results_csv))
    save_csv(grid_rows, Path(args.grid_results_csv))
    print(f"[DONE] {format_name(args.bit_width)} qat_models={len(train_rows)}, grid_rows={len(grid_rows)}")
