...
def default_output_dir(bit_width: int) -> str:
    return f"./dscnn_kws/quantization/qat_int{bit_width}_noise_snr_scene_models"


def default_train_csv(bit_width: int) -> str:
    return f"./dscnn_kws/quantization/qat_int{bit_width}_noise_snr_scene_train_results.csv"


def default_grid_csv(bit_width: int) -> str:
    return f"./dscnn_kws/quantization/qat_int{bit_width}_noise_snr_scene_grid_results.csv"
...
def make_lowbit_qconfig(bit_width: int) -> QConfig:
    ...
    return QConfig(activation=fake_quant, weight=fake_quant)
    ...
    *,
    ...
    common = {
        ...
        "qat": {
            ...
            "scope": (
                f"frontend {tag} fake-quant QAT + DSCNN backbone {tag} fake-quant simulation"
                if args.quantize_frontend
                else f"DSCNN backbone {tag} fake-quant simulation; frontend remains float"
            ),
        },
        "note": (
    ...
    for scene_idx, scene in enumerate(args.scene_names):
        ...
        for snr_idx, snr_db in enumerate(args.test_snrs):
            ...
            row = {
                ...
                "bit_width": args.bit_width,
                "quantized_backend": "fake_quant_float_ops",
            }
            rows.append(row)
            print(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=f"INT{BIT_WIDTH} QAT fake-quant fine-tuning for noise-scene DSCNN checkpoints.")
