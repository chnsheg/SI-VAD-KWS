...
from dscnn_kws.frontend import Int8MFCCFrontend
...
PATTERN = "*.pt"
CHECKPOINTS = None
OUTPUT_DIR = "./dscnn_kws/quantization/qat_int8_mfcc_frontend_models"
TRAIN_RESULTS_CSV = "./dscnn_kws/quantization/qat_int8_mfcc_frontend_train_results.csv"
GRID_RESULTS_CSV = "./dscnn_kws/quantization/qat_int8_mfcc_frontend_grid_results.csv"
LIMIT = 0
...
# constraints during QAT, then is replaced by Int8MFCCFrontend for final eval.
QAT_EPOCHS = 10
LR = 5e-5
WEIGHT_DECAY = 1e-6
BACKEND = "fbgemm"
...
# Int8MFCCFrontend. Use natural_log + PWL to match the TorchMFCC PWL frontend
# used during QAT. If you need to mimic torchaudio.transforms.MFCC instead,
# switch INT8_MFCC_MEL_LOG_MODE back to "torchaudio_db".
INT8_MFCC_MEL_LOG_MODE = "natural_log"  # torchaudio_db or natural_log
INT8_MFCC_LOG_APPROX_MODE = "pwl"  # exact or pwl
...
INT8_MFCC_DB_AMIN = 1e-10
INT8_MFCC_TOP_DB = 80.0
...
# with INT8_MFCC_LOG_* when you want training and final eval to see the same
# log approximation family.
LOG_APPROX_MODE = "pwl"
...
class Int8MFCCQuantizedBackboneModel(nn.Module):
    """Connect an integer-aware MFCC frontend to a converted INT8 backbone."""

    def __init__(self, frontend: Int8MFCCFrontend, quantized_backbone: nn.Module, dct_coeff: int):
        super().__init__()
...
def build_int8_mfcc_frontend(args: argparse.Namespace, *, observer_enabled: bool) -> Int8MFCCFrontend:
    n_fft = int(args.sample_rate * args.window_size_ms / 1000)
    hop_length = int(args.sample_rate * args.window_stride_ms / 1000)
    return Int8MFCCFrontend(
        ...
        mel_log_mode=args.int8_mfcc_mel_log_mode,
        ...
        db_amin=args.int8_mfcc_db_amin,
        top_db=args.int8_mfcc_top_db,
...
@torch.no_grad()
def calibrate_int8_mfcc_frontend(
    args: argparse.Namespace,
    *,
    dataset: str,
    scale_json: Path,
    device: torch.device,
) -> tuple[Int8MFCCFrontend, int]:
    frontend = build_int8_mfcc_frontend(args, observer_enabled=True).to(device)
    frontend.eval()
    loader = build_calibration_loader(args, dataset)

    batches = 0
    for waveform, _labels in tqdm(loader, desc="calibrate-int8-mfcc", leave=False):
        frontend(waveform.to(device))
        batches += 1
        if args.calibration_batches > 0 and batches >= args.calibration_batches:
            break

    scale_json.parent.mkdir(parents=True, exist_ok=True)
    ...
    frontend = Int8MFCCFrontend.from_scale_json(scale_json).cpu().eval()
    ...
    for scene_idx, scene in enumerate(args.scene_names):
        ...
        for snr_idx, snr_db in enumerate(args.test_snrs):
            ...
            row = {
                ...
                "int8_mfcc_scale_json": str(int8_mfcc_scale_json),
...
def save_artifacts(
    *,
    ...
    inference_path = output_dir / f"{stem}_int8_mfcc_int8_backbone.pt"
    common = {
        "source_checkpoint": str(source_checkpoint),
        "metadata": row,
        "int8_mfcc_scale_json": str(int8_mfcc_scale_json),
        ...
        "frontend_quantization": {
            "format": "integer-aware int8 mfcc reference",
            "coeff_bits": args.int8_mfcc_coeff_bits,
            "mel_log_mode": args.int8_mfcc_mel_log_mode,
            "log_approx_mode": args.int8_mfcc_log_approx_mode,
            "requantize_power": args.int8_mfcc_requantize_power,
            "requantize_mel": args.int8_mfcc_requantize_mel,
            "scale_json": str(int8_mfcc_scale_json),
        },
    }
    ...
    torch.save(
        {
            **common,
            ...
            "format": "int8_mfcc_frontend_plus_converted_int8_backbone",
        },
        inference_path,
    )
...
def run_one_checkpoint(path: Path, args: argparse.Namespace, device: torch.device) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    scale_json = output_dir / f"{stem}_int8_mfcc_scales.json"
    int8_frontend, calibration_batches = calibrate_int8_mfcc_frontend(
        args,
        dataset=dataset,
    ...
    inference_model = Int8MFCCQuantizedBackboneModel(
        frontend=int8_frontend,
        quantized_backbone=quantized_qat_model.backbone,
        dct_coeff=args.dct_coeff,
    ).cpu()
    inference_model.eval()
    int8_mfcc_test_m = evaluate(inference_model, test_list_loader, torch.device("cpu"))
    print(
        "[TEST] "
        ...
        f"int8_mfcc_int8_backbone_tau_list_acc={int8_mfcc_test_m.acc:.4f} f1={int8_mfcc_test_m.f1:.4f}"
    )
    ...
    artifact_row = {
        ...
        "int8_mfcc_int8_backbone_test_tau_list_acc": int8_mfcc_test_m.acc,
        "int8_mfcc_int8_backbone_test_tau_list_f1": int8_mfcc_test_m.f1,
        "backend": args.backend,
        "frontend": "int8_mfcc",
        "int8_mfcc_scale_json": str(scale_json.resolve()),
        "int8_mfcc_calibration_batches": calibration_batches,
        "int8_mfcc_mel_log_mode": args.int8_mfcc_mel_log_mode,
        "int8_mfcc_log_approx_mode": args.int8_mfcc_log_approx_mode,
        "int8_mfcc_requantize_power": args.int8_mfcc_requantize_power,
        "int8_mfcc_requantize_mel": args.int8_mfcc_requantize_mel,
    }
    ...
    qat_path, quantized_backbone_path, inference_path = save_artifacts(
        ...
        int8_mfcc_scale_json=scale_json,
        row=artifact_row,
        args=args,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="QAT DSCNN backbone, then evaluate true-ish INT8 MFCC frontend on TAU scenes.")
    ...
    parser.add_argument("--int8_mfcc_mel_log_mode", choices=["torchaudio_db", "natural_log"], default=INT8_MFCC_MEL_LOG_MODE)
    ...
    parser.add_argument("--int8_mfcc_db_amin", type=float, default=INT8_MFCC_DB_AMIN)
    parser.add_argument("--int8_mfcc_top_db", type=float, default=INT8_MFCC_TOP_DB)
